...
export default function ProductoLista1() {
  const { loading, data: productos } = useFetch('http://192.168.100.250:3000/productos')
                      ...
                      renderItem={({ item }) =>
                          <ListItems
                              onPress={() => navigation.navigate('producto-detalle', 
                                { id: item.id, nomProducto: item.nomProducto, precio: item.precio, cantidad: item.cantidad })}
                              id={item.id}
